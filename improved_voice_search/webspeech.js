...
function WebSpeechRecognition() {
  ...
  // Sets the object which will receive ranking list.
  r.rankingList = function(id) {
    r.ranking_list = r.getElem(id);
  };
  ...
  r.start = function() {
    ...
    r.final_transcript = '';
    r.inner_text = '';
    r.new_result_array =new Array();
    r.setText(r.final_results, '');
    r.setText(r.interim_results, '');
    r.setText(r.ranking_list, '');
  ...
  r.onStatusChange = function(key) {
    if (!!r.status_elem) {
      ...
      if (s) {
        ...
        r.status_elem.style.display = 'block';
      } else {
        r.status_elem.style.display = 'none';
      }
    }
  };
    ...
    r.recognition.onresult = function(event) {
      ...
      for (var i = event.resultIndex; i < event.results.length; ++i) {
        ...
        if (event.results[i].isFinal) {
          ...
          //shows the top 5 alternatives
          if(event.results[i].length>2){
            r.inner_text += '<tr><td class=\"fontNote\">Do you mean:</td>';
          }
            for(var n=1; n<event.results[i].length;n++){   
                if(n===1){
                    r.inner_text +='<td id=\"alternative_'+n+'\"><a href=\"#\" class=\"fontAlternatives\" onclick=\"changeQuery(\'alternative_'+n+'\')\">'+event.results[i][n].transcript+'</a></td></tr>';
                }else{
                    r.inner_text +='<tr><td></td><td id=\"alternative_'+n+'\"><a href=\"#\" class=\"fontAlternatives\" onclick=\"changeQuery(\'alternative_'+n+'\')\">'+event.results[i][n].transcript+'</a></td></tr>'; 
                }
            }
        } else {
      ...
      r.setText(r.ranking_list, r.inner_text);
...
var status_array_en = {
  ...
  'active' : 'Speak now. Click on the microphone to finish.',
...
var image_array_mic = {
  'ready' : 'images/Button.png',
  'complete' : 'images/Button.png',
  'active' : 'images/ButtonRecording.gif',
  'no-speech' : 'images/ButtonFail.png',
  'disable' : 'images/ButtonFail.png'
};
